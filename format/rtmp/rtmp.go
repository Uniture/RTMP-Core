...
func NewPushConn(netconn net.Conn, url *url.URL) (*Conn, error) {
	conn := NewConn(netconn)
	conn.isserver = false
	conn.URL = url
	...
	for {
		...
		if self.gotcommand {
			...
			case "publish":
				...
				if cberr != nil {
					err = fmt.Errorf("RTMPRTMP: OnPlayOrPublish check failed")
					return
				}
			...
			case "play":
				if Debug {
					fmt.Println("RTMPRTMP: < play")
				}

				if len(self.commandparams) < 1 {
					err = fmt.Errorf("RTMPRTMP: command play params invalid")
					return
				}
...
func (self *Conn) handleMsg(timestamp uint32, msgsid uint32, msgtypeid uint8, msgdata []byte) (err error) {
	...
	// log.Println("Got message: ", len(msgdata))
